...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'ip') {
    msg.reply('connect 185.193.165.100:2701 ');
  }
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    .setDescription("xry Reklam sistemi, " + `***${msg.guild.name}***` + " adlı sunucunuzda reklam yakaladım.")
...
client.on(`guildMemberAdd`, async member => {
  const e = new Discord.RichEmbed()
    ...
    .addField(`𝑩𝒖𝒓𝒂𝒚𝒂𝑺𝒖𝒏𝒖𝒄𝒖𝒎𝒖𝒛𝒂 𝑯𝒐ş𝒈𝒆𝒍𝒅𝒊𝒏 𝒃𝒖 𝒃𝒐𝒕 ''⚜ 𝑲𝒐̈𝒏𝒊𝒈🔱#4653'𝒆ğ𝒍𝒆𝒏𝒄𝒆 𝒂𝒎𝒂ç𝒍ı 𝒚𝒂𝒛ı𝒍𝒎ış𝒕ı𝒓 𝒌𝒆𝒚𝒇𝒊𝒍𝒊 𝒈𝒖̈𝒏𝒍𝒆𝒓 𝒗𝒆 𝒌𝒆𝒚𝒊𝒇𝒍𝒊 𝒐𝒚𝒖𝒏𝒍𝒂𝒓 𝒅𝒊𝒍𝒆𝒓𝒊𝒎 `, `𝒉𝒐ş𝒈𝒆𝒍𝒅𝒊𝒏`)
    .setFooter(` `)
  member.send(e);
});
...
const ms = require('parse-ms')
  ...
  const e = new Discord.RichEmbed()
    ...
    .addField(`Sunucumuza hoşgeldin kanka `, `Mesaj`)
...
function play(guild, song) {
  ...
  ///botu sunucuya girdiğinde sunucu sahibine msj atar///  
client.on("guildCreate", async guild => {
  const girismesaj = [
    "Bot sunucuna eklendi.Tebrikler dostum.",
    "Bu bot **⚜ König🔱#4653** tarafından geliştirilmektedir.",
    'iyi günlerde kullan..'
  ];
...
client.on("messageUpdate", msg => {
 ...
 const i = db.fetch(`${msg.guild.id}.kufur`)
    ...
    ///botun dm görme//
client.on("message", msg => {
  var dm = client.channels.get("715325453560184954"); //mesajın geleceği kanal idsi//
...
client.on('message', (message) => {
  if(message.content.toLocaleLowerCase() === '640497223502790667         ') {
    message.react('👍')
  }
})
...
client.on('guildMemberAdd', async (member) => {
    ...
    kanal.send(`**Heeey! ${member} Sunucumuza Hoşgeldin bu bot ''⚜ König🔱#4653'eğlence amaçlı yazılmıştır keyfili günler ve keyifli oyunlar dilerim ! - *   Seninle Beraber ${member.guild.memberCount} Kişi Olduk!`) ;
})

///botu sesli odada tutma///
client.on("ready", () => {
  client.channels.get("710129371557658705").join();///sayıların olduğu kısma kanal id giriniz
   
})
    const express = require('express')
const app = express()
// Pudochu ^-^
app.get('/', (req, res) => res.send('xry | Aktif!')) // sitenize girdiğinde görebilirsiniz.
...
55
56
client.on('message', async message => {// chimp#0110
if(message.author.bot) return;
...
const asd = await require('quick.db').fetch(`yazı.${message.guild.id}.${message.author.id}`)

if(!asd) {
message.delete()
require('quick.db').set(`yazı.${message.guild.id}.${message.author.id}`, '1')
return message.channel.send(`Çok fazla reklam yapıyorsun: 1/3`) }

if(asd == 1) {
message.delete()
require('quick.db').set(`yazı.${message.guild.id}.${message.author.id}`, '2')
return message.channel.send(`Çok fazla reklam yapıyorsun: 2/3`) }
  
if(asd == 2) {
message.delete()
message.guild.members.get(message.author.id).ban()
require('quick.db').delete(`yazı.${message.guild.id}.${message.author.id}`)
return message.guild.channels.get('log kanal id').send(new Discord.RichEmbed().setColor('RED').setDescription(`${message.author}, 3 kere reklam yaptığı için banlandı.`)) }

}
})// codare

client.on('messageUpdate', async (oldMessage, newMessage) => {// chimp#0110
